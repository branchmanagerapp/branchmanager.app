import { createClient } from "https://esm.sh/@supabase/supabase-js@2.45.1";
const SUPABASE_URL = Deno.env.get("SUPABASE_URL") ?? "";
const SERVICE_KEY = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") ?? "";
const CLIENT_ID = Deno.env.get("BOUNCIE_CLIENT_ID") ?? "";
const CLIENT_SECRET = Deno.env.get("BOUNCIE_CLIENT_SECRET") ?? "";
const TOKEN_URL = Deno.env.get("BOUNCIE_TOKEN_URL") ?? "https://auth.bouncie.com/oauth/token";
const BOUNCIE_API_BASE = (Deno.env.get("BOUNCIE_API_BASE") ?? "https://api.bouncie.dev/v1").replace(/\/+$/, "");
function pointsFromTrip(t) {
  const out = [];
  const coords = t.gps?.coordinates ?? [];
  const stamps = t.gps?.timestamps ?? [];
  const speeds = t.gps?.speeds ?? [];
  if (stamps.length) {
    const n = Math.min(coords.length, stamps.length);
    for(let i = 0; i < n; i++){
      const c = coords[i];
      const ts = stamps[i];
      if (!c || c.length < 2 || !ts) continue;
      out.push({
        ts,
        lon: c[0],
        lat: c[1],
        speed_mph: typeof speeds[i] === "number" ? speeds[i] : null
      });
    }
    return out;
  }
  // Verified Aug 5 2026: Bouncie's gps-format=geojson carries ONLY
  // coordinates — no per-point timestamps — so the branch above yielded
  // zero points for every trip (root cause of the silent no-insert sync).
  // Interpolate linearly between the trip's startTime and endTime instead.
  // In-progress trips (no endTime yet) are skipped; the next 15-min tick
  // picks them up once complete.
  const startMs = Date.parse(t.startTime ?? "");
  const endMs = Date.parse(t.endTime ?? "");
  if (!coords.length || !Number.isFinite(startMs) || !Number.isFinite(endMs) || endMs < startMs) return out;
  const n = coords.length;
  for(let i = 0; i < n; i++){
    const c = coords[i];
    if (!c || c.length < 2) continue;
    const ts = new Date(startMs + (n === 1 ? 0 : (endMs - startMs) * i / (n - 1))).toISOString();
    out.push({
      ts,
      lon: c[0],
      lat: c[1],
      speed_mph: typeof t.averageSpeed === "number" ? t.averageSpeed : null
    });
  }
  return out;
}
async function refreshBouncieToken(refreshToken) {
  if (!CLIENT_ID || !CLIENT_SECRET) return null;
  const body = new URLSearchParams({
    grant_type: "refresh_token",
    refresh_token: refreshToken,
    client_id: CLIENT_ID,
    client_secret: CLIENT_SECRET
  });
  const r = await fetch(TOKEN_URL, {
    method: "POST",
    headers: {
      "Content-Type": "application/x-www-form-urlencoded",
      Accept: "application/json"
    },
    body
  });
  if (!r.ok) {
    console.error("BOUNCIE_REFRESH_FAIL", r.status, (await r.text()).slice(0, 200));
    return null;
  }
  const j = await r.json();
  if (!j.access_token) return null;
  const expiresIn = j.expires_in ?? 3600;
  return {
    access_token: j.access_token,
    refresh_token: j.refresh_token ?? refreshToken,
    expires_at: new Date(Date.now() + expiresIn * 1000).toISOString(),
    scope: j.scope ?? ""
  };
}
async function bouncieGet(token, path) {
  const url = `${BOUNCIE_API_BASE}${path.startsWith("/") ? path : "/" + path}`;
  const res = await fetch(url, {
    headers: {
      Authorization: token,
      Accept: "application/json"
    }
  });
  if (!res.ok) {
    const body = await res.text().catch(()=>"");
    console.error(`BOUNCIE_REST ${res.status} ${url}`, body.slice(0, 300));
    return {
      ok: false,
      status: res.status,
      data: null
    };
  }
  return {
    ok: true,
    status: res.status,
    data: await res.json()
  };
}
async function syncTenant(sb, tenantId, cfg) {
  let token = cfg.access_token;
  // Pre-refresh if expired (or expires in <5 min) and we have refresh_token
  const expSoon = cfg.expires_at && new Date(cfg.expires_at).getTime() < Date.now() + 5 * 60 * 1000;
  if (expSoon && cfg.refresh_token) {
    const refreshed = await refreshBouncieToken(cfg.refresh_token);
    if (refreshed) {
      token = refreshed.access_token;
      await saveTenantBouncie(sb, tenantId, refreshed);
    }
  }
  // Fetch vehicles, retry once after refresh if 401
  let vResp = await bouncieGet(token, "/vehicles");
  if (!vResp.ok && vResp.status === 401 && cfg.refresh_token) {
    const refreshed = await refreshBouncieToken(cfg.refresh_token);
    if (refreshed) {
      token = refreshed.access_token;
      await saveTenantBouncie(sb, tenantId, refreshed);
      vResp = await bouncieGet(token, "/vehicles");
    }
  }
  if (!vResp.ok) return {
    tenantId,
    error: `vehicles ${vResp.status}`,
    inserted: 0
  };
  const raw = vResp.data;
  const vehicles = Array.isArray(raw) ? raw : raw?.vehicles ?? [];
  let totalInserted = 0;
  const perVehicle = [];
  const allVisits = [];
  const { data: tenantRow } = await sb.from("tenants").select("config").eq("id", tenantId).maybeSingle();
  const visitExclusions = tenantRow?.config?.visit_exclusions || [];
  for (const bv of vehicles){
    const imei = bv.imei;
    if (!imei) continue;
    let { data: localVehicle } = await sb.from("vehicles").select("id, tenant_id, last_seen_at, name").eq("tenant_id", tenantId).eq("tracker_device_id", imei).maybeSingle();
    if (!localVehicle && bv.vin) {
      const { data: byVin } = await sb.from("vehicles").select("id, tenant_id, last_seen_at, name").eq("tenant_id", tenantId).eq("vin", bv.vin).maybeSingle();
      localVehicle = byVin;
      if (localVehicle) {
        await sb.from("vehicles").update({
          tracker_device_id: imei,
          tracker_provider: "bouncie"
        }).eq("id", localVehicle.id);
      }
    }
    if (!localVehicle) continue;
    const { data: latestPing } = await sb.from("vehicle_positions").select("ts").eq("vehicle_id", localVehicle.id).order("ts", {
      ascending: false
    }).limit(1).maybeSingle();
    const dayAgo = new Date(Date.now() - 24 * 3600 * 1000).toISOString();
    const startsAfter = latestPing?.ts && latestPing.ts > dayAgo ? latestPing.ts : dayAgo;
    const tResp = await bouncieGet(token, `/trips?imei=${encodeURIComponent(imei)}&starts-after=${encodeURIComponent(startsAfter)}&gps-format=geojson`);
    if (!tResp.ok) {
      // Surface the failure — a silent {inserted:0} is indistinguishable
      // from "no trips today" and hid the May–Aug outage.
      perVehicle.push({
        imei,
        inserted: 0,
        latestSeen: null,
        error: `trips HTTP ${tResp.status}`
      });
      continue;
    }
    const rawT = tResp.data;
    const trips = Array.isArray(rawT) ? rawT : rawT?.trips ?? [];
    let inserted = 0;
    let latestSeenTs = null;
    for (const trip of trips){
      const pts = pointsFromTrip(trip);
      if (!pts.length) continue;
      const fresh = pts.filter((p)=>!latestPing?.ts || p.ts > latestPing.ts);
      if (!fresh.length) continue;
      const rows = fresh.map((p)=>({
          tenant_id: localVehicle.tenant_id,
          vehicle_id: localVehicle.id,
          ts: p.ts,
          lat: p.lat,
          lon: p.lon,
          speed_mph: p.speed_mph ?? null,
          source: "bouncie-rest"
        }));
      for(let i = 0; i < rows.length; i += 500){
        const slice = rows.slice(i, i + 500);
        const { error } = await sb.from("vehicle_positions").insert(slice);
        if (error) {
          console.error("BOUNCIE_INSERT_ERR", imei, error.message);
          break;
        }
        inserted += slice.length;
      }
      const lastTs = fresh[fresh.length - 1].ts;
      if (!latestSeenTs || lastTs > latestSeenTs) latestSeenTs = lastTs;
    }
    if (latestSeenTs && (!localVehicle.last_seen_at || latestSeenTs > localVehicle.last_seen_at)) {
      await sb.from("vehicles").update({
        last_seen_at: latestSeenTs
      }).eq("id", localVehicle.id);
    }
    totalInserted += inserted;
    for (const vis of detectVisits(trips, visitExclusions)) {
      allVisits.push({ tenant_id: tenantId, vehicle_id: localVehicle.id,
        vehicle_name: localVehicle.name || imei, ...vis,
        address: null, matched_client_id: null, matched_client_name: null, matched_job_id: null });
    }
    perVehicle.push({
      imei,
      inserted,
      latestSeen: latestSeenTs,
      trips: trips.length
    });
  }
  let visitsNew = 0;
  if (allVisits.length) {
    const { data: fresh, error: vErr } = await sb.from("vehicle_visits")
      .upsert(allVisits, { onConflict: "vehicle_name,arrived_at", ignoreDuplicates: true })
      .select("id, lat, lon");
    if (vErr) console.warn("visit upsert failed:", vErr.message);
    else if (fresh?.length) {
      visitsNew = fresh.length;
      const { data: clientRows } = await sb.from("clients")
        .select("id, name, address").eq("tenant_id", tenantId);
      const keys = {};
      for (const c of clientRows || []) { const k = streetKey(c.address); if (k) keys[k] = c; }
      for (const row of fresh) {
        const addr = await reverseGeocode(row.lat, row.lon);
        const patch = { address: addr };
        const m = keys[streetKey(addr)];
        if (m) { patch.matched_client_id = m.id; patch.matched_client_name = m.name; }
        await sb.from("vehicle_visits").update(patch).eq("id", row.id);
        await new Promise((r) => setTimeout(r, 1100));
      }
    }
  }
  return {
    tenantId,
    vehicles_seen: vehicles.length,
    inserted: totalInserted,
    visits_new: visitsNew,
    perVehicle
  };
}

// ── v2 visit detection (Doug Aug 5: trucks tell the story, no logins) ──
// A gap of 60min–12h between consecutive trips = a visit at the earlier
// trip's end coordinate. Exclusions (yard/home) come from
// tenants.config.visit_exclusions: [{lat, lon, radius_m, label}].
function haversineM(lat1, lon1, lat2, lon2) {
  const R = 6371000, p = Math.PI / 180;
  const a = Math.sin((lat2 - lat1) * p / 2) ** 2
    + Math.cos(lat1 * p) * Math.cos(lat2 * p) * Math.sin((lon2 - lon1) * p / 2) ** 2;
  return 2 * R * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
}
function streetKey(s) {
  const m = /(\d+)[ ,]+([A-Za-z' ]+?)(?:,| Road| Rd| Street| St| Lane| Ln| Drive| Dr| Ave| Court| Ct| Place| Pl|$)/.exec(s || "");
  return m ? (m[1] + " " + m[2].trim().toLowerCase()) : null;
}
function detectVisits(trips, exclusions) {
  const sorted = [...trips].sort((a, b) => (a.startTime < b.startTime ? -1 : 1));
  const visits = [];
  for (let i = 0; i < sorted.length - 1; i++) {
    const endT = Date.parse(sorted[i].endTime), nextT = Date.parse(sorted[i + 1].startTime);
    const gapMin = (nextT - endT) / 60000;
    if (!(gapMin >= 60 && gapMin <= 720)) continue;
    const coords = sorted[i].gps?.coordinates || [];
    const last = coords[coords.length - 1];
    if (!last) continue;
    const lon = last[0], lat = last[1];
    if ((exclusions || []).some((e) => e && typeof e.lat === "number"
        && haversineM(lat, lon, e.lat, e.lon) <= (e.radius_m || 300))) continue;
    visits.push({ arrived_at: sorted[i].endTime, departed_at: sorted[i + 1].startTime,
      minutes: Math.round(gapMin), lat, lon });
  }
  return visits;
}
async function reverseGeocode(lat, lon) {
  try {
    const r = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lon}&zoom=18`,
      { headers: { "User-Agent": "BranchManager/1.0 (info@branchmanager.app)" } });
    if (!r.ok) return "";
    return (await r.json())?.display_name || "";
  } catch (_e) { return ""; }
}

async function saveTenantBouncie(sb, tenantId, cfg) {
  const { data: tenant } = await sb.from("tenants").select("config").eq("id", tenantId).maybeSingle();
  const nextConfig = {
    ...tenant?.config ?? {}
  };
  nextConfig.bouncie = {
    ...nextConfig.bouncie ?? {},
    ...cfg
  };
  await sb.from("tenants").update({
    config: nextConfig
  }).eq("id", tenantId);
}
Deno.serve(async (req)=>{
  if (req.method === "OPTIONS") {
    return new Response("ok", {
      headers: {
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "POST, OPTIONS",
        "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type"
      }
    });
  }
  const sb = createClient(SUPABASE_URL, SERVICE_KEY);
  // Find every tenant with a Bouncie token configured
  const { data: tenants, error } = await sb.from("tenants").select("id, config").not("config->bouncie", "is", null);
  if (error) {
    return new Response(JSON.stringify({
      ok: false,
      error: error.message
    }), {
      status: 500,
      headers: {
        "Content-Type": "application/json"
      }
    });
  }
  if (!tenants?.length) {
    return new Response(JSON.stringify({
      ok: true,
      message: "No tenants have a Bouncie token configured.",
      next: "Visit https://auth.bouncie.com/dialog/authorize?client_id=<BOUNCIE_CLIENT_ID>&redirect_uri=<callback>&response_type=code&state=<tenant_id> to bootstrap."
    }), {
      status: 200,
      headers: {
        "Content-Type": "application/json"
      }
    });
  }
  const results = [];
  for (const t of tenants){
    const cfg = t.config?.bouncie;
    if (!cfg?.access_token) continue;
    try {
      results.push(await syncTenant(sb, t.id, cfg));
    } catch (e) {
      results.push({
        tenantId: t.id,
        error: String(e)
      });
    }
  }
  return new Response(JSON.stringify({
    ok: true,
    tenants: results.length,
    results
  }), {
    status: 200,
    headers: {
      "Content-Type": "application/json"
    }
  });
});