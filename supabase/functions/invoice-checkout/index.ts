/**
 * Branch Manager — Single-invoice Stripe Checkout
 *
 * The FIX for the broken "prefilled_quantity" pay-link trick (Stripe ignores
 * that URL param, so texted buy.stripe.com links always showed $0.50). This
 * mints a proper Checkout Session for ONE invoice's exact amount — the same
 * mechanism portal-pay-all already uses successfully.
 *
 * Anon-callable by design: the pay link is meant to be shared with the customer,
 * and all this does is let someone PAY a specific invoice (not read/modify data).
 *
 * Request body: { invoiceId, returnUrl? }
 * Returns: { ok, url, amount }
 *
 * Deploy: supabase functions deploy invoice-checkout --no-verify-jwt
 */
...
serve(async (req) => {
  ...
  try {
    ...
    const invRes = await pgFetch(
      `/rest/v1/invoices?id=eq.${encodeURIComponent(invoiceId)}&select=id,invoice_number,balance,total,status,tenant_id,client_id,client_email,client_name&limit=1`
    );
    ...
    if (inv.status === "paid") return json(400, { ok: false, error: "Invoice is already paid" });
    if (inv.status === "cancelled") return json(400, { ok: false, error: "Invoice was cancelled" });
    ...
    if (cents < 50) return json(400, { ok: false, error: "Amount must be at least $0.50" });
    ...
    if (!secretKey) return json(400, { ok: false, error: "Stripe not configured for tenant" });
    ...
    return json(200, { ok: true, url: checkout.url, amount: (cents / 100).toFixed(2), invoiceNumber: invNum });
  } catch (err) {
